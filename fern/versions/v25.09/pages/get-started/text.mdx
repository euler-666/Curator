...
<Note>
For this example, you'll need sample JSONL files in `~/nemo_curator/data/sample/`. Each line should be a JSON object with at least `text` and `id` fields. You can create test data or refer to [Read Existing Data](/curate-text/load-data/read-existing) and [Data Loading](/curate-text/load-data) for information on downloading data.
</Note>
