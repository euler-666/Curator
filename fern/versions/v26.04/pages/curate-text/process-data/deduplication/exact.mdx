...
- Use `input_blocksize="2GiB"` for optimal performance with the default batch size
- Increase `identification_batchsize` to improve shuffle throughput when GPU memory allows. For example, set `identification_batchsize=4` with `input_blocksize="256MiB"` to process ~1 GB per insertion call
